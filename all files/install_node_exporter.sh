...
echo ""
sleep 2
if curl -s http://localhost:9100/metrics | head -5 > /dev/null 2>&1; then
    log_ok "Metrics endpoint is responding on port 9100."
    echo ""
    echo "Sample metrics:"
    curl -s http://localhost:9100/metrics | grep -E "^node_cpu_seconds_total|^node_memory_MemTotal|^node_filesystem_avail" | head -5
else
    echo "WARNING: Metrics endpoint not responding yet. Check: journalctl -u node_exporter"
fi
...
echo ""
echo "  Give your Prometheus teammate this scrape target:"
